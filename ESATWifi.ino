...
boolean wifiConnect(){  
  ...
  while (WiFi.status() != WL_CONNECTED) {
    if (millis()-t1>5000);{return false;}
    delay(500);
  }
  delay(100);//ESP.reset();
  return true;
...
void processCMD(String cmd){
  ...
  if (key=="config"){
  int i1;
  String temp;String remainder=value;
  ...
  WriteConfig(cmd);
  delay(50);
  if(wifiConnect()){Serial.println("#s:2");} else {Serial.println("#s:1");}//Wifi connected
  }
  else if (key=="reset"){
  //ESP.reset();  
  }
...
void loop() {
  if(WiFi.status()==3){wifiStatus=1;} 
    //else {Serial.println("#server_nok");delay(5000);}  
  ...
  else {Serial.println("#s:3");//connected to server
    client.print("{\"type\":\"login\",\"id\":\""+String(config.id)+"\"}");
    }
  }  
  ...
  if(identifier=="@"){
  String msg="{\"type\":\"onPacket\",\"data\":\""+toHex(config.id,1)+cmd.substring(1,cmd.length()+1)+"\"}";
  client.print(msg);
  }
  else if(identifier=="&"){
  String msg="{\"type\":\"log\",\"data\":\""+toHex(config.id,1)+cmd.substring(1,cmd.length()+1)+"\"}";
  client.print(msg);
  }
  else if(identifier=="#"){
  processCMD(cmd);
